...
const checkTagsCompliance = (requirement, serviceDescription) => {
  ...
  console.debug('SD: ', serviceDescription)
  ...
  const requiredTags = _.map(requirement, (tagValue, tagName) => {
    console.debug(
      'PARSING REQUIRED TAGS -- TAG VALUE: ',
      tagValue,
      'TAG NAME: ',
      tagName,
    )
  ...
  console.debug('REQUIRED TAGS: ', requiredTags)
  console.debug('PARSED TAGS: ', parsedTags)

  const nonCompliantTags = _.filter(requiredTags, (tagObj) => {
    console.debug('TAG OBJ: ', tagObj)
